#
# this file was automatically generated by `rake bootstrap' at Thu Sep 18 19:59:13 +1000 2008.
# do not modify
#
...
def eChar
c = nil
_or(proc { (_applyWithArgs("exactly","\\");c = _apply("char");Hash['n' => "\n", 't' => "\t", 'r' => "\r", '\'' => "'", '\"' => '"', '\\' => '\\'][c]) }, proc { _apply("char") })
end
...
_or(proc { _apply("innerMatchedBraces") }, proc { _apply("nonBraceChar") })
end

def innerMatchedBraces
xs = nil
(_applyWithArgs("exactly","{");xs = _xmany { _apply("inside") };_applyWithArgs("exactly","}");"{#{xs.join('')}}")
end

def outerMatchedBraces
xs = nil
(_applyWithArgs("exactly","{");xs = _xmany { _apply("inside") };_applyWithArgs("exactly","}");xs.join(''))
end

def newStyleSemAction
x = nil
(_apply("spaces");x = _apply("outerMatchedBraces");p 'using new style sem action' => x; x)
end

def semAction

_or(proc { _apply("newStyleSemAction") }, proc { _apply("oldStyleSemAction") })
end
end
